...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        boolean leftTreeBounds = leftTree <= last;
        boolean rightTreeBounds = rightTree <= last;

        // Check if the root has a child, if not no need to siftDown.
        if (leftTreeBounds || rightTreeBounds) {

            int smallest = 0;
            if (leftTreeBounds && rightTreeBounds) {
                // If there are two children compare which is smaller.
                if (order.compare(array[leftTree], array[rightTree]) < 0) {
                    smallest = leftTree;
                } else {
                    smallest = rightTree;
                }
            } else if (leftTreeBounds) {
            ...
            // Only sift down when the root is greater than the child.
            if (order.compare(array[top], array[smallest]) > 0) {
                // "Sift down" by swapping entries.
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * Recursively call until we are at the smallest case which will satisfy
         * heap requirements for siftDown.
         */
        if (leftTree <= smallest) {
            heapify(array, leftTree, order);
        }
        if (rightTree <= smallest) {
            heapify(array, rightTree, order);
        }
        siftDown(array, top, smallest, order);
    }
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        int count = 0;
        // Add all elements from the queue into an array.
        while (q.length() > 0) {
        ...
        /*
         * Build a heap from the array by heapifying it, starting at the "top"
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        // TODO - fill in body
        // Default cases set by the ensures clause.
    ...
    public final void add(T x) {
        ...
        // TODO - fill in body
        // In insertion mode order does not matter, add elements to a queue.
    ...
    public final void changeToExtractionMode() {
        ...
        // TODO - fill in body
        // Set insertionMode to false.
        this.insertionMode = false;

        // Set heapSize equal to entries in the queue that is not empty yet.
        this.heapSize = this.entries.length();

        // Build a heap with the entries in the queue and our comparator.
    ...
    public final T removeFirst() {
        ...
        // TODO - fill in body
        // Grab the "top" or first element in the array.
        T first = this.heap[0];

        /*
         * Decrease the instance variable size, purely mental. Size of the array
         * does not change.
         */
        this.heapSize--;

        /*
         * Since the "size" has been decrease swap the element into a position
         * that is considered garbage.
         */
    ...
    public final int size() {
        ...
        // TODO - fill in body
        // When in insertionMode the length is the size of the queue.
        int size = this.entries.length();
        if (!this.insertionMode) {
            // If we are in extractionMode it is the size of the array.
            size = this.heapSize;
        }
